...
**Status:** v1.6.3 — 72/72 unit × 3 TZ + 36/36 e2e green · live E2E + API contracts re-verified on prod (cycle 6) · security headers (CSP/HSTS) + 413 payload guard · lighthouse a11y/bp/seo 1.0 · live https://car-care.jesspete.shop (env-driven SEO) · git invariants + content-as-data + CI coverage guarded by `scripts/skill-verify.sh` (11 checks) · CI: documented gate runs on every push (`.github/workflows/verify-gate.yml`) · cycle-5 audit: PASS (`docs/code-review-audit-2026-09-cycle5.md`) — SEO parity + hardening · cycle-6: live E2E re-verify + `og:locale` parity (G6) + audit round 4 PASS (`docs/code-review-audit-2026-09-cycle6.md`) — B1 day-picker business-TZ anchor (PRD F2.2 now code-true), B2 stats content-as-data fix
...
- **F5.1** Metadata: title/description, `metadataBase` env-driven (`NEXT_PUBLIC_SITE_URL`/`SITE_URL`, fallback `https://car-care.jesspete.shop`; original ref `https://wecarecarcare.com`), self-referencing **canonical** (`alternates.canonical`), OG/Twitter cards referencing `/images/hero-car.webp` with `og:locale: en_US` (SEO parity G6), `viewport.themeColor` #0a0b0d.
...
- [ ] `npm test` 72/72 × 3 TZ · `bun run e2e` 36/36 on standalone :3100 · `bun run lint` clean · `bunx tsc --noEmit` clean (src+e2e) · `bun run build` green with asset copy · `bun audit --prod` 0
